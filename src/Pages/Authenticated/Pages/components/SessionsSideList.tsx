...
const SessionsSideList: React.FC<SessionsSideListPropsInterface> = (_props: SessionsSideListPropsInterface) => {
  ...
  const sessionsOfInterest = props.sessions
    .filter(s => s.direction === SessionDirection.PROVIDED);
  const mappedSessionCards = sessionsOfInterest
    .map(s => (
      <SessionBlock country={s.providerCountry}
